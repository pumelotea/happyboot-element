...
export default defineComponent ({
  ...
  setup() {
    ...
    const handleSearch = async () => {
      ...
      const res: any = await context.$api.subjectPage(tableData.value.searchCondition)
      if (res.code === 0) {
    ...
    const handleDelete = (row: any) => {
      context.$confirm('即将删除该条数据, 是否确认?', '提示', {
        ...
        .then(() => {
          context.$api.subjectDelete(row.id).then((res: any) => {
            if (res.code === 0) {
              tableData.value.searchCondition.pageNo = 1
              handleSearch()
              context.$notify({
    ...
    const handleMultiDelete = () => {
      context.$confirm(
        ...
        .then(() => {
          ...
          context.$api.subjectDelete(ids.substr(1)).then((res: any) => {
            if (res.code === 0) {
              tableData.value.searchCondition.pageNo = 1
              handleSearch()
              context.$notify({
