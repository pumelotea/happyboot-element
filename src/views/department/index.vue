...
export default defineComponent ({
  ...
  setup() {
    ...
    const getNode = async (id: any = 0, success: any) => {
      const res: any = await context.$api.getDeptTreeByParentId(id)
      if (res.code === 0) {
        success(res.data)
      }
    }
    ...
    const moveNode = async (params: any, done: any) => {
      const res: any = await context.$api.modifyDeptNode(params)
      if (res.code !== 0) {
        context.$notify({
          title: '失败',
          message: res.msg,
    ...
    const removeNode = async (id: any, done: any) => {
      const res: any = await context.$api.deleteDeptNode(id)
      if (res.code === 0) {
        context.$notify({
          title: '成功',
          message: res.msg,
